...
Windows 的顺序为显式配置、默认目录、注册表兜底。注册表只在 Windows 兜底函数内导入，依次读取当前用户及本机的 64/32 位卸载条目；匹配 WorkBuddy 产品名及可选版本号，优先使用有效的 InstallLocation，再读取 DisplayIcon。路径必须完整、文件名必须为 WorkBuddy.exe 且为现存文件；正确解析引号、路径中的逗号、图标索引与 REG_EXPAND_SZ 环境变量。失效条目及权限错误不阻断后续搜索，路径按大小写不敏感去重，提前找到时也关闭注册表句柄。
...
注册表测试使用内存模拟，不写系统注册表；覆盖探测优先级、用户/本机及双视图、路径解析、条目损坏、权限拒绝、重复路径、句柄回收，以及无 winreg 模块时脚本仍可导入。
